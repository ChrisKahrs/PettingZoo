...
class CustomEnvironment(ParallelEnv):
    ...
    def render(self, rewards):
        ...
        grid[self.guard_y-1, self.guard_x-1] = 2
        grid[self.escape_y, self.escape_x] = 9

        if rewards["prisoner"] != 0 or rewards["guard"] != 0:
            print(f"{grid} step: {self.timestep}")
...
if __name__ == "__main__":
    parallel_api_test(CustomEnvironment(), num_cycles=100)
